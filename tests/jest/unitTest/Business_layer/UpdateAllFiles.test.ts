...
// import { tagUpdater, getSingleFileTagEdits } from "@components/DicomData/TagUpdater"; 
...
//         await updateAllFiles(dicomData, true, newTagValues, files, currentFileIndex, "single");
...
//         expect(getSingleFileTagEdits).toHaveBeenCalledTimes(2); // Verify function call
//     });
// });
import { updateAllFiles } from "@components/DicomData/UpdateAllFiles";
import {
...
describe("updateAllFiles", () => {
    ...
    test("should process files and create a zip when downloadOption is 'zip'", async () => {
        await updateAllFiles(dicomData, true, newTagValues, files, currentFileIndex, "zip");

        expect(tagUpdater).toHaveBeenCalledTimes(2);
        expect(getSingleFileTagEdits).toHaveBeenCalledTimes(2);
        expect(createFile).toHaveBeenCalledTimes(2);
        expect(createZipFromFiles).toHaveBeenCalledTimes(1);
        expect(downloadDicomFile).toHaveBeenCalledWith({ name: "updateDicoms.zip", content: "mockZipContent(2)" });
    });

    test("should process files correctly when series is false", async () => {
        await updateAllFiles(dicomData, false, newTagValues, files, currentFileIndex, "single");
    ...
    test("should handle empty dicomData gracefully", async () => {
        await updateAllFiles([], true, newTagValues, files, currentFileIndex, "single");
