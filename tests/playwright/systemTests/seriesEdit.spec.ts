...
test('Edit tag in series', async ({ page }) => {
    try {
        ...
        await page.waitForSelector('button:has-text("Apply Edits to All Files")', {
            state: "visible",
            timeout: 5000,
        });
        ...
        const dicomFile = fs.readdirSync(extractDir)
            .filter(file => file.endsWith('.dcm'))
        ...
        while (hasMoreFiles) {
            ...
            const PatientNameRow = page.locator('tr', {
                has: page.locator('td', { hasText: 'PatientName' })
            }).first();
            ...
            const patientNameValue = await PatientNameRow.locator('td').nth(2).textContent();
            debug(`File ${fileCount} - PatientName value: ${patientNameValue}`);

            expect(patientNameValue).toContain("New Value");
            debug(`File ${fileCount} - Verified: PatientName contains the new value`);
